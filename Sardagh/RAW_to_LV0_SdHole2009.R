...
## loop 1 over years
#############################################################################
for (year in 2009:aktuell){#2009:aktuell
  #############################################################################
  ## step 1.04
  ...
  #############################################################################
  inz.path<-paste0(path$w[path$n=="RAW.p"],"SdHole2009/")
  ...
  if(year==2019){# change 1460 when you change the cables back
  ...
  write.table(db.sdhole[as.numeric(format(as.POSIXct(db.sdhole[,1],format='%Y-%m-%d %H:%M',origin=origin, tz = "UTC"),format='%Y'))==year,-2],
              paste0(path$w[path$n=="LV0.p"],"SdHole2009/00_full_dataset/SdHole2009_",year,"_lv0.dat"),quote = F,dec=".",sep=",",row.names=F)

  cat("#\n# level0 SdHole2009: ",year,"without problems!\n#\n")
} # end loop over years
