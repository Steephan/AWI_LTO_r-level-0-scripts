...
for (year in run.year){#2013:2016 2012:aktuell
  ...
  ###..........................................................................
  inz.01.path<-paste0(p.1$w[p.1$n=="RAW.p"],"KuMet2010/01_air/")
  files.01<-list.files(inz.01.path,pattern="*.dat")
  
  inz.02.path<-paste0(p.1$w[p.1$n=="RAW.p"],"KuMet2010/02_soil/")
  files.02<-list.files(inz.02.path,pattern="*.dat")
  
  inz.03.path<-paste0(p.1$w[p.1$n=="RAW.p"],"KuMet2010/03_wind/")
  ...
  write.table(db.kumet.air[as.numeric(format(as.POSIXct(db.kumet.air[,1],format='%Y-%m-%d %H:%M',origin=origin, tz = "UTC"),format='%Y'))==year,-2],
              paste0(p.1$w[p.1$n=="LV0.p"],"KuMet2010/01_met/kumet2010_met_",year,".dat"),quote = F,dec=".",sep=",",row.names=F)
  ...
  write.table(db.kumet.soil[as.numeric(format(as.POSIXct(db.kumet.soil[,1],format='%Y-%m-%d %H:%M',origin=origin, tz = "UTC"),format='%Y'))==year,],
              paste0(p.1$w[p.1$n=="LV0.p"],"KuMet2010/02_soil/kumet2010_soil_",year,".dat"),quote = F,dec=".",sep=",",row.names=F)
