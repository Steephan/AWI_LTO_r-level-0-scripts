...
##
##   by: Stephan.Lange@awi.de,  Marie.Ecker@awi.de
...
for (t.year in run.year) {
   ...
   #############################################################################
  ## step 1.04
  ## set 2 empty tables with length of t.year
  ...
  #############################################################################
  inz.path <- paste0(path$w[path$n == "RAW.p"], "TVCSoil2016/")
  ...
  for (i in 1:length(files2read)) {#1:length(files2read)
    ...
    #############################################################################
    ## step 1.09
    ## check file if dates are in running t.year of loop 1
    #############################################################################
  ...
  write.table(db.TVCsoil[as.numeric(format(as.POSIXct(db.TVCsoil[, 1], format = '%Y-%m-%d %H:%M', origin = origin, tz = "UTC"), format = '%Y')) == t.year, -2],
              paste0(path$w[path$n == "LV0.p"], "TVCSoil2016/00_full_dataset/TVCSoil2016_", t.year, "_lv0.dat"), quote = F, dec = ".", sep = ",", row.names = F)


} # end loop over t.years

cat("\n#\n# TVCsoil2016 without problems!\n#\n")
