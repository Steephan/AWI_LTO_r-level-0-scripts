...
## -----
##
##  grundsaetzlich: die RAW daten wechseln staendig!!!
##  es bestehen mindestens 10 verschiedene tabellen formate !!!!!
##
##  sehr haesssssssslich!!!!!!
##
##  Grundsaetzlier Aufbau:
##    a) bestimme pfade und files die f?r das Jahr relevant sind
##    b) gehe nur die relvanten files durch (Schleife)
##    c) einlesen,  checken,  if-clause bestimmen,  Aufbau  .... MERGE
...
for (year in 2019:aktuell) {#2002:aktuell
  ...
  #############################################################################
  if (year < 2007) {
    inz.01.path <- paste0(path$w[path$n == "RAW.p"], "SaMet2002/06_SAK_2002-2009/")
    files.01 <- list.files(inz.01.path, pattern = "*.dat")
  } else if (year %in% c(2007, 2008)) {
    inz.01.path <- paste0(path$w[path$n == "RAW.p"], "SaMet2002/06_SAK_2002-2009/")
    files.01 <- list.files(inz.01.path, pattern = "*.dat")
    inz.02.path <- paste0(path$w[path$n == "RAW.p"], "SaMet2002/07_SaMet2007/")
    ...
    inz.02.path <- paste0(path$w[path$n == "RAW.p"], "SaMet2002/07_SaMet2007/")
    files.02 <- list.files(inz.02.path, pattern = "*.dat")
    inz.03.path <- paste0(path$w[path$n == "RAW.p"], "SaMet2002/06_SAK_2009-2014/")
    files.03 <- list.files(inz.03.path, pattern = "*.dat")
    files.01 <- c("sak09_001-039.dat", files.02, files.03[1:3])
  } else if (year %in% c(2010)) {
    inz.01.path <- paste0(path$w[path$n == "RAW.p"], "SaMet2002/07_SaMet2007/")
    files.01 <- list.files(inz.01.path, pattern = "*.dat")
    inz.02.path <- paste0(path$w[path$n == "RAW.p"], "SaMet2002/06_SAK_2009-2014/")
    files.02 <- list.files(inz.02.path, pattern = "*.dat")
    files.01 <- c(files.01, files.02)
  } else if (year %in% c(2011, 2012, 2013)) {
    inz.01.path <- paste0(path$w[path$n == "RAW.p"], "SaMet2002/06_SAK_2009-2014/")
    files.01 <- list.files(inz.01.path, pattern = "*.dat")
  } else if (year == 2014) {
    inz.2014.path <- paste0(path$w[path$n == "RAW.p"], "SaMet2002/06_SAK_2009-2014/")
    files.2014 <- list.files(inz.2014.path, pattern = "*.dat")[19]   # "201408180400_201308221200_SaMet2002_SAK_CR1000_13760.dat"
    inz.01.path <- paste0(path$w[path$n == "RAW.p"], "SaMet2002/01_SaMeteo/")
    files.01 <- list.files(inz.01.path, pattern = "*.dat")
    inz.02.path <- paste0(path$w[path$n == "RAW.p"], "SaMet2002/02_SaRad/")
    files.02 <- list.files(inz.02.path, pattern = "*.dat")
    inz.03.path <- paste0(path$w[path$n == "RAW.p"], "SaMet2002/03_SaSoil/")
    files.03 <- list.files(inz.03.path, pattern = "*.dat")
    m <- 0
  } else if (year > 2014) {
    inz.01.path <- paste0(path$w[path$n == "RAW.p"], "SaMet2002/01_SaMeteo/")
  ...
  #  main input tasks             main nput tasks

  #############################################################################
  ## step 1.06a -----
  ## loop 2 over all SaMeteo files
  #############################################################################
  ...
  for (i in (1:length(files.01))) {# SaMet2010 SaMeteo 1:length(files.01)
    ...
    if (spalten.met == 27) {# 2010-2013
      ...
      dada.meteo$Ts_6 <- NA
      # data rad
      dada.meteo$RadNet <- dada.meteo$RadNet * 1000 / 15.4     # Pete: Sensitivity: 15.4 µV/(Wm2) @ Stephan: alt: dada.meteo$RadNet*100 das scheint auch nicht richtig zu sein!!!
      dada.meteo$SwIn <- NA
      dada.meteo$SwOut <- NA
      dada.meteo$LwIn_rawcor <- NA
      dada.meteo$LwOut_rawcor <- dada.meteo$LwOut_raw * 1000 / 15.07 # Pete: Sensitivity: 15.07 µV/(Wm2) // should be Bolzmann canstant following manual
    ...
    } else if (spalten.met == 12 & year %in% c(2007, 2008, 2009, 2010)) {# 2009-2010
      cat("zock 12", year, files.01[i], "\n")
      colnames(dada.meteo) <- c("UTC", "Ts_1", "Ts_6", "Ts_3", "WT_raw",
                              "vwc_WT", "Vm_1_Avg", "Vm_2_Avg", "R_1_Avg", "R_2_Avg", # werden nicht genutzt
                              "Tair_a_50", "RH_50")
      ...
      dada.rad                   <- dada.meteo[, c("UTC", "Ts_1")]# why Ts_1????? ... egal welche spalte,  es muss nur eine dummi-spalte vorhanden sein
  ...
  #############################################################################
  # - [CS616_PA_uS] < 19 --> kein Wert --> entweder WL zu niedrig oder einfrierend,  auftauend
  # - [CS616_PA_uS] < 27 --> diese Formel verwenden: (0.01831394*[CS616_PA_uS]^3-1.2398*[CS616_PA_uS]^2+28.84699187*[CS616_PA_uS]-224.42499308)
  # - [CS616_PA_uS] > 27 --> diese Formel verwenden: (0.06194726*[CS616_PA_uS]^2-1.7673294*[CS616_PA_uS]+13.66709591)
  ...
  write.table(db.samet, paste0(path$w[path$n == "LV0.p"], "SaMet2002/00_full_dataset/SaMet2002_", year, "_lv0.dat"), quote = F, dec = ".", sep = ",", row.names = F)
  #
  write.table(db.samet.meteo[, -c(16, 18)], paste0(path$w[path$n == "LV0.p"], "SaMet2002/01_meteo/SaMet2002_Meteo_", year, "_lv0.dat"), quote = F, dec = ".", sep = ",", row.names = F)
  write.table(db.samet.rad, paste0(path$w[path$n == "LV0.p"], "SaMet2002/02_radiation/SaMet2002_Rad_", year, "_lv0.dat"), quote = F, dec = ".", sep = ",", row.names = F)
  write.table(db.samet.soil, paste0(path$w[path$n == "LV0.p"], "SaMet2002/03_soil/SaMet2002_Soil_", year, "_lv0.dat"), quote = F, dec = ".", sep = ",", row.names = F)
  #
