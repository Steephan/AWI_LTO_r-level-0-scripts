...
{# files
inz.path<-"/home/christoph/Dokumente/AWI/SaLakes/SaLakes/SaLake1_FishLake/" # adjust
...
# WORKAROUND FOR MISSING 0m SENSOR
#~ data.0m<-data.frame(UTC=data.2m[,1],Tw_0=rep(NA,dim(data.2m)[1]))
...
{# some checks and corrections NOT NEEDED AT FISH
...
#~ unique(format(data.0m[,1],"%M-%S")) # empty here

#~ unique(format(data.2m[,1],"%M-%S")) # check, good
...
{### P_0 -> water level calculation: waterlevel = (P_0 - PA)/Rho_water # NO P_0 DATA HERE!!!
...
#############################################################################
{# put all together
minmaxdate<-range(data.2m[,1],data.4m[,1]) # ,na.rm=T
...
db.salakes<-Reduce(function(x,y) merge(x,y,all.x=T),list(db.salakes,data.2m,data.4m)) # ,data.6m
#~ db.salakes<-db.salakes[-which(duplicated(db.salakes[,1])),] # delete duplicates NOT NEEDED HERE

# workaround
db.salakes<-data.frame(UTC=db.salakes[,1],P_0=as.numeric(rep(NA,dim(data.2m)[1])),Tw_0=as.numeric(rep(NA,dim(data.2m)[1])),db.salakes[,-1])
...
png("/tmp/Sa_Lake_1.png",height=600,width=900)
...
for (year in 2009:aktuell){
  ...
  write.table(db.salakes[as.numeric(format(as.POSIXct(db.salakes[,1],format='%Y-%m-%d %H:%M',origin=origin, tz = "UTC"),format='%Y'))==year,],
              paste0("/tmp/Sa_Lake1_",year,".dat"),quote = F,dec=".",sep=",",row.names=F)
