...
{# files
inz.path<-"/home/christoph/Dokumente/AWI/SaLakes/SaLakes/SaLake4_Banja2Lake/" # adjust
...
dataset1.crop<-dataset1[,c(1,which(lake=="Banja2Lake"))]
...
data.6m<-NULL
for(i in files2read[grep("6m",files2read)]){
#~   i<-files2read[grep("6m",files2read)][1] # testing
  temp<-read.table(paste0(inz.path,i),sep=",",dec=".",header=F,skip=2,fill=TRUE,na="NAN")
#~ 	flag_proto<-grep("Protokolliert",apply(temp[,-(1:4)],1,paste,collapse="")) # which lines contain protocoled flag
  temp<-temp[-grep("Protokolliert",apply(temp[,-(1:4)],1,paste,collapse="")),2:3] # kicked out record number
  temp[,1]<-as.POSIXct(temp[,1],format='%Y.%m.%d %H:%M:%S', tz = "UTC") # kicked out as.numeric
  colnames(temp)<-c("UTC","Tw_600") # water + air pressure
  data.6m<-rbind(data.6m,temp)
}
}
...
#~ unique(format(data.2m[,1],"%M-%S"))
temp<-colnames(data.2m)
data.2m<-data.frame(round(data.2m[,1],"hours"),data.2m[,-1])
colnames(data.2m)<-temp
rm(temp)

#~ unique(format(data.4m[,1],"%M-%S"))
temp<-colnames(data.4m)
data.4m<-data.frame(round(data.4m[,1],"hours"),data.4m[,-1])
colnames(data.4m)<-temp
rm(temp)

#~ unique(format(data.6m[,1],"%M-%S"))
temp<-colnames(data.6m)
data.6m<-data.frame(round(data.6m[,1],"hours"),data.6m[,-1])
colnames(data.6m)<-temp
rm(temp)
...
#############################################################################
{# put all together
minmaxdate<-range(data.0m[,1],data.2m[,1],data.4m[,1],data.6m[,1]) # ,na.rm=T
...
db.salakes<-Reduce(function(x,y) merge(x,y,all.x=T),list(db.salakes,data.0m,data.2m,data.4m,data.6m)) # ,data.6m
...
png("/tmp/Sa_Lake_4.png",height=600,width=900)
plot(db.salakes[,c(1,2)],type="l",ylim=c(-5,30),ylab="")
lines(db.salakes[,c(1,3)],col=2)
lines(db.salakes[,c(1,4)],col=3)
lines(db.salakes[,c(1,5)],col=4)
lines(db.salakes[,c(1,6)],col=5)
dev.off()
...
for (year in 2009:aktuell){
  ...
  write.table(db.salakes[as.numeric(format(as.POSIXct(db.salakes[,1],format='%Y-%m-%d %H:%M',origin=origin, tz = "UTC"),format='%Y'))==year,],
              paste0("/tmp/Sa_Lake4_",year,".dat"),quote = F,dec=".",sep=",",row.names=F)
