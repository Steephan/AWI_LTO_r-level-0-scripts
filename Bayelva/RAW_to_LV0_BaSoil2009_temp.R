...
for (year in run.year) {
  ...
  db.now[, c(2:11)] <- NA
  db.now[, 12] <- 0
  files.temp <- dir(paste0(path$w[path$n == "BaS.raw3.p"]), pattern = glob2rx("*.dat"))

  if (year >= 2015) {
    files.temp <- c(files.temp, dir(paste0(path$w[path$n == "BaS.onl.p"]), pattern = glob2rx("*TempOnline.dat")))
  }

  for (lola in 1:length(files.temp)) {
    inz.path <- paste0(path$w[path$n == "BaS.raw3.p"])
  ...
  write.table(db.now, paste0(paste0(path$w[path$n == "LV0.p"]) , "BaSoil2009/01_temperature/BaSoil2009_Ts_", year, "_lv0.dat"),
